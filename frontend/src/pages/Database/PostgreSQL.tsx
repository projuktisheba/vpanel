...
export default function Postgresql() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await databaseManager.listPostgresqlUsers();
  ...
  const fetchDatabases = async () => {
    if (!openCreateDBModal && openCreateUserModal) setLoading(true);
    try {
      const res = await databaseManager.listPostgresqlDB();
      // res here is the whole API response, so extract the databases array
  ...
  const handleConfirmClearDatabase = async () => {
    ...
    try {
      const resp = await databaseManager.clearPostgresqlDB(
  ...
  const handleConfirmDeleteDatabase = async () => {
    ...
    try {
      const resp = await databaseManager.deletePostgresqlDB(
  ...
  const createNewDatabase = (e: FormEvent) => {
    ...
    (async () => {
      ...
      const data = await databaseManager.createPostgresqlDB(databaseName, username);
      console.log(data);
  ...
  const createNewUser = (e: FormEvent) => {
    ...
    (async () => {
      ...
      const data = await databaseManager.createPostgresqlUser(
        newUsername,
        newPassword
      );
      console.log(data);
  ...
  const handleImportDatabase = async () => {
    ...
    try {
      ...
      // Call backend
      const response = await databaseManager.importPostgresqlDB(formData);
  ...
  return (
    <>
      <PageMeta title="Postgresql" description="Manage Postgresql Database" />
      <PageBreadcrumb pageTitle="Manage Postgresql Database" />
      <ComponentCard>
            ...
            tabs={[
              {
                ...
                content: (
                  <DataTable
                    ...
                    title={"List of Postgresql Database"}
                    extraActions={
                      <Button
                        size="sm"
                        variant="outline"
            ...
            <div className="no-scrollbar relative w-full max-w-[500px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
              ...
              <div className="px-2">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Enter a database name and assign a user to create a new Postgresql
                  database.
                </p>
              </div>
          ...
          >
            <div className="no-scrollbar relative w-full max-w-[500px] rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
              ...
              <div className="px-2">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Fill in the details to create a new Postgresql database user. Make
